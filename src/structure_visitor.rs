...
impl<'ps> StructureVisitor<'ps> {
    ...
    pub fn write_element_do<FN>(&mut self,
                                ident: &str,
    ...
    pub fn write_element_handled<FN>(&mut self,
                                     ident: &str,
                                     kind: StructureElementKind,
                                     sourceRange: SourceRange,
                                     type_desc: String,
        ...
        where FN: Fn(&mut Self)
    {
        ...
        match self.write_element_do(ident, kind, sourceRange, type_desc, walkFn) {
            ...
            Err(error) => {
                std::io::stderr()
                    .write_fmt(format_args!("Error writing element: {}", error))
                    .ok();
            }
        }
    }

    pub fn write_element_TODO<FN>(&mut self,
                                  ident: Ident,
                                  kind: StructureElementKind,
                                  span: Span,
                                  walkFn: FN)
    ...
    pub fn write_element<FN>(&mut self,
                             ident: Ident,
                             kind: StructureElementKind,
                             span: Span,
                             type_desc: String,
        ...
        where FN: Fn(&mut Self)
    {
        self.write_element_handled(&*ident.name.as_str(),
                                   kind,
    ...
    fn write_function_element(&mut self,
                              ident: Ident,
                              span: Span,
                              fd: &FnDecl,
                              walkFn: &Fn(&mut Self)) {
        ...
        self.write_element(ident,
                           StructureElementKind::Function,
                           span,
                           type_desc,
                           walkFn);
    }
...
impl<'v> Visitor for StructureVisitor<'v> {
    ...
    fn visit_item(&mut self, item: &Item) {
        ...
        let walkFn: &Fn(&mut Self) = &|_self: &mut Self| { walk_item(_self, item); };
        ...
        match item.node {
            ...
            ItemKind::Fn(ref declaration, unsafety, constness, abi, ref generics, ref body) => {
                self.visit_fn(FnKind::ItemFn(item.ident,
                                             generics,
                                             unsafety,
                                             constness,
                                             abi,
            ...
            ItemKind::Impl(_,
                           _,
                           ref _type_parameters,
                           ref _opt_trait_reference,
                           ref _typ,
                           ref _impl_items) => {
                kind = StructureElementKind::Impl;
            }
    ...
    fn visit_struct_field(&mut self, sf: &StructField) {
        if let Some(ident) = sf.ident {
            self.write_element_TODO(ident,
                                    StructureElementKind::Var,
                                    sf.span,
                                    |_self: &mut Self| { walk_struct_field(_self, sf); });
        }
    }
    ...
    fn visit_foreign_item(&mut self, foreign_item: &ForeignItem) {
        ...
        self.write_element_TODO(foreign_item.ident,
                                kind,
                                foreign_item.span,
                                |_self: &mut Self| { walk_foreign_item(_self, foreign_item); });
    }
...
fn tests_describe_structure() {
    ...
    fn test_describe_structure(source: &str, expected: &str) {
...

        {
            ...
            parse_describe::write_parse_analysis_contents(messages, elements, &mut tokenWriter)
                .ok()
                .unwrap();
        }
    ...
    test_describe_structure("extern crate xx;",
                            r#"ExternCrate { "xx" { 0:0 0:16 } {} "" {} }"#);

    test_describe_structure("const xx : u32 = 1;",
                            r#"Var { "xx" { 0:0 0:19 } {} "u32" {} }"#);


    test_describe_structure("mod myMod   ;  ",
                            r#"Mod { "myMod" { 0:0 0:13 } {} "" {} }"#);
  ...
  Var { "xx" { 0:12 0:32 } {} "u32" {} }
    ...
    test_describe_structure("fn xx() { }",
                            r#"Function { "xx" { 0:0 0:11 } {} "()" {} }"#);
    ...
    test_describe_structure("type MyType = &u32<asd>;",
                            r#"TypeAlias { "MyType" { 0:0 0:24 } {} "" {} }"#);
  ...
  Var { "foo" { 0:16 0:25 } {} "" {} }
    ...
    test_describe_structure("trait MyTrait { } ",
                            r#"Trait { "MyTrait" { 0:0 0:17 } {} "" {} }"#);
    test_describe_structure(
...
r#"Impl { "MyTrait" { 0:0 0:75 } {} "" {}
  ...
  Var { "foo" { 0:54 0:73 } {} "" {} }
    ...
    */
    ...
    test_describe_structure("use blah as foo;",
                            r#"Use { "blah as foo" { 0:0 0:16 } {} "" {} }"#);
    // TODO: this is not printing the global path prefix, seems to be a limitation from libsyntax?
    test_describe_structure("use ::blah::foo as myfoo;",
                            r#"Use { "::blah::foo as myfoo" { 0:0 0:25 } {} "" {} }"#);
    test_describe_structure("use ::blah::foo::*;",
                            r#"Use { "::blah::foo::*" { 0:0 0:19 } {} "" {} }"#);
    test_describe_structure(
        "use blah::foo:: { One as OtherOne, self as Two };",
    ...
    test_describe_structure("fn foo() { my_macro!(asf); }",
                            r#"Function { "foo" { 0:0 0:28 } {} "()" {} }"#);
